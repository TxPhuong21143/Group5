...
public class AppServices implements IAppServices {
    ...
    @Autowired
    private AccountShipContactRepo accountShipContactRepo;
    @Autowired
    ...
    public CreateAccountData createAccountAndAccountShipContact(CreateAccountData accountData) {
        ...
        if(!accountRepo.checkUserName(accountData.getUserName()).isEmpty()){
        ...
        AccountStatus statusOnline = accountStatusRepo.findById(1).get();
        account.setAccountStatus(statusOnline);
        Role role =roleRepo.findById(3).get();
        account.setRole(role);
        accountRepo.save(account);
        AccountShipContact createShipContact = new AccountShipContact();
        createShipContact.setAccountDetailAddress(account.getAccountDetailAddress());
        createShipContact.setAccount(account);
        createShipContact.setAccountPhoneNumber(account.getSdt());
        createShipContact.setReceiverName(account.getName());
        accountShipContactRepo.save(createShipContact);
